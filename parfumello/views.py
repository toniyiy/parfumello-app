...
class CreatePaymentIntentView(APIView):
    ...
    def post(self, request):
        ...
        for item in items:
            ...
            if not perfume:
                return Response({'error': f'Perfume not found.'}, status=status.HTTP_400_BAD_REQUEST)
...
class PlaceOrderView(APIView):
    ...
    def post(self, request):
        ...
        if metadata_user_id != str(request.user.id):
            return Response({'error': 'Invalid payment.'}, status=status.HTTP_403_FORBIDDEN)

        existing = models.Order.objects.filter(stripe_payment_intent_id=payment_intent_id).first()
        if existing:
...
class OrderListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return models.Order.objects.filter(user=self.request.user).prefetch_related('items').order_by('-created_at')
